...
func testNoticesExists(t *testing.T) {
	...
	e, err := NoticeExists(ctx, tx, o.NoticeID)
	if err != nil {
...
func testNoticesFind(t *testing.T) {
	...
	noticeFound, err := FindNotice(ctx, tx, o.NoticeID)
	if err != nil {
		t.Error(err)
	}
...
var (
	noticeDBTypes = map[string]string{`NoticeID`: `int`, `NoticeTitle`: `varchar`, `NoticeContent`: `varchar`, `NoticeCreatedAt`: `datetime`, `NoticeIsEmergency`: `tinyint`}
	_             = bytes.MinRead
)
