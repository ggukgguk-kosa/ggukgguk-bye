...
func testReplyToOneRecordUsingRecord(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadRecord(ctx, tx, false, (*[]*Reply)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testReplyToOneMemberUsingMember(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := ReplySlice{&local}
	if err = local.L.LoadMember(ctx, tx, false, (*[]*Reply)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testReplyToOneSetOpRecordUsingRecord(t *testing.T) {
	var err error
...
func testReplyToOneSetOpMemberUsingMember(t *testing.T) {
	var err error
...
var (
	replyDBTypes = map[string]string{`ReplyID`: `int`, `ReplyContent`: `varchar`, `ReplyDate`: `datetime`, `RecordID`: `int`, `MemberID`: `varchar`}
	_            = bytes.MinRead
)
...
func testRepliesUpsert(t *testing.T) {
	...
	if len(mySQLReplyUniqueColumns) == 0 {
