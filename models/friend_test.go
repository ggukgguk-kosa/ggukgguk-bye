...
func testFriendsExists(t *testing.T) {
	...
	e, err := FriendExists(ctx, tx, o.FriendID)
	if err != nil {
...
func testFriendsFind(t *testing.T) {
	...
	friendFound, err := FindFriend(ctx, tx, o.FriendID)
	if err != nil {
		t.Error(err)
	}
...
func testFriendToOneMemberUsingMember1(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.Member1ID = foreign.MemberID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Member1().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadMember1(ctx, tx, false, (*[]*Friend)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Member1 == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Member1 = nil
	if err = local.L.LoadMember1(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Member1 == nil {
		t.Error("struct should have been eager loaded")
...
func testFriendToOneMemberUsingMember2(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.Member2ID = foreign.MemberID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.Member2().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := FriendSlice{&local}
	if err = local.L.LoadMember2(ctx, tx, false, (*[]*Friend)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Member2 == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.Member2 = nil
	if err = local.L.LoadMember2(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Member2 == nil {
		t.Error("struct should have been eager loaded")
...
func testFriendToOneSetOpMemberUsingMember1(t *testing.T) {
	var err error
	...
	for i, x := range []*Member{&b, &c} {
		...
		if x.R.Member1Friends[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.Member1ID != x.MemberID {
			t.Error("foreign key was wrong value", a.Member1ID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.Member1ID))
		reflect.Indirect(reflect.ValueOf(&a.Member1ID)).Set(zero)
		...
		if a.Member1ID != x.MemberID {
			t.Error("foreign key was wrong value", a.Member1ID, x.MemberID)
		}
	}
}
func testFriendToOneSetOpMemberUsingMember2(t *testing.T) {
	var err error
	...
	for i, x := range []*Member{&b, &c} {
		...
		if x.R.Member2Friends[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.Member2ID != x.MemberID {
			t.Error("foreign key was wrong value", a.Member2ID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.Member2ID))
		reflect.Indirect(reflect.ValueOf(&a.Member2ID)).Set(zero)
		...
		if a.Member2ID != x.MemberID {
			t.Error("foreign key was wrong value", a.Member2ID, x.MemberID)
		}
	}
}
...
var (
	friendDBTypes = map[string]string{`FriendID`: `int`, `Member1ID`: `varchar`, `Member2ID`: `varchar`, `FriendCreatedAt`: `datetime`}
	_             = bytes.MinRead
)
